...
export const wpp_recordatorio = (user) => {
    return `COLEGIO ACADÉMICO EL POBLADO: Cordial saludo. Se informa que a la fecha, el estudiante ${user.first_name} ${user.last_name} (Código: ${user.student.code}), de grado ${user.student.grade.name}, adeuda un valor total de $ ${formatNumber(user.student.amountOwed)}, por concepto de ${user.student.monthOwed} mensualidad(es) atrasadas. Se le recuerda al acudiente que los pagos deben realizarse los primeros 5 días del mes y se debe enviar la foto del comprobante de pago a este WhatsApp para su regitro. Recomendamos efectuar el pago del valor adeudado lo más pronto posible. Agradecemos su atención y quedamos atentos a una pronta respuesta.`
}